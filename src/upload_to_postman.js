...
module.exports = async (postmanKey, app, collectionName, workspaceId) => {
  ...
  const body = (expressObject, postmanBody) => {
    if (expressObject && (expressObject.body || expressObject.sampleRequest))
      return {
        mode: `raw`,
        raw:
  ...
  const postManId = postmanObject => {
    if (postmanObject && postmanObject.data && postmanObject.data._postman_id) {
      return {
    ...
    return Object.assign(
      postManId(existingPostManRoute),
      {
        ...
        request: Object.assign({
          ...
          body: body(
            expressRoute,
            existingPostManRoute.data && existingPostManRoute.data.request.body
      ...
      tests(
        expressRoute,
        existingPostManRoute.data ? existingPostManRoute.data.event : null
      )
    );
  };
